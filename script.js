...
async function exportPdf() {
  const h2c = (window.html2canvas && typeof window.html2canvas === 'function')
            ? window.html2canvas
            : (window.html2canvas && typeof window.html2canvas.default === 'function')
              ? window.html2canvas.default
              : null;
  const { jsPDF } = (window.jspdf || {});
  if (!h2c || !jsPDF) { alert('PDF libraries not loaded. Refresh and try again.'); return; }

  
  const captureNode =
    document.getElementById('actionsRow')?.closest('form') ||
    document.querySelector('form:has(#check), form:has(#questionnairePermalink)') ||
    document.querySelector('form') ||
    document.body;

  const canvas = await h2c(captureNode, { useCORS: true, background: '#ffffff', scale: 2 });
  ...
  pdf.text('Exempt Development Check — Result', margin, 50);

  pdf.setFont('helvetica', 'normal'); pdf.setFontSize(11);
  pdf.text('Generated: ' + new Date().toLocaleString(), margin, 70);

  pdf.addImage(canvas.toDataURL('image/png'), 'PNG', margin, 90, imgW, imgH);

  pdf.setFontSize(10);
  pdf.text('Generated by Albury City Exempt Development Checker (SEPP 2008)',
           margin, 100 + imgH + 20);

  pdf.save('Exempt_Development_Result.pdf');
}


(function injectPdfButtonOnce() {
  document.addEventListener('DOMContentLoaded', () => {
    
    const checkBtn = document.getElementById('check') ||
                     document.querySelector('button#check');
    const permaBtn = document.getElementById('questionnairePermalink') ||
                     document.querySelector('#questionnairePermalink');

    
    let host = (permaBtn && permaBtn.parentElement) ||
               (checkBtn && checkBtn.parentElement) ||
               // fallback: create a row at end of the active form
    ...
    // Global toggler used by your PASS/FAIL logic
    window.togglePdf = (show) => {
